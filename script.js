...
const inventors = [
  ...
  const countInstances = data.reduce((acc, data) => {
  ...
  const age = devs.some((dev) => new Date().getFullYear() - dev.year >= 19);

  console.log(age)
  // Array.prototype.every()
  // 9. Check if everyone is 19 or older?
  const everyonesAge = devs.every((dev) => new Date().getFullYear() - dev.year >= 19);

  console.log(everyonesAge);
